...
def get_running_logs(cursor,conn):
    ...
    if(total_runs > pagination_size):
        ...
        for i in range(int(rounds)):
            start_index = (i * pagination_size) + 1
            end_index = start_index + pagination_size
            page_content = df.loc[start_index:end_index-1,['date','distance_km']].to_string()
            print(page_content)
            print(f"\nShowing runs {start_index} to {min(end_index -1, total_runs)} of {total_runs} runs.\n")
            if i < rounds - 1:
                pagination_input = input("Press Enter to see more runs, \"D\" to delete a run or \"Q\" to quit to the main menu.")
                if pagination_input.lower() == 'q':
                    break
                elif pagination_input.lower() == 'd':
                    delete_run(page_content,cursor,conn,df)
                    break
            else:
                pagination_input = input("Press Enter to return to the main menu or \"D\" to delete a run.")
            if pagination_input.lower() == 'd':
                delete_run(page_content,cursor,conn,df)
                break
    else:
        page_content = df.loc[:,['date','distance_km']].to_string()
        print(page_content)
        print(f"\nShowing all {total_runs} runs.\n")
        list_input = input("Press Enter to return to the main menu or \"D\" to delete a run.")
        if list_input.lower() == 'd':
            delete_run(page_content,cursor,conn,df)

def delete_run(page_content,cursor,conn,df):
    while True:
        print(page_content)
        delete_input = input("Enter the ID of the run you would like to delete: ")
        if delete_input.isdigit(): #TODO - this needs to have some validation put in that the input is one of the ones displayed above.
            break
        else:
            print(f'{delete_input} is not a valid choice, plese choose a valid number')
    
    delete_id = df.loc[int(delete_input),'id']
    delete_date = df.loc[int(delete_input),'date']
    delete_distance = df.loc[int(delete_input),'distance_km']
    confirm_input = input(f'Are you sure you want to delete {delete_distance}km run on {delete_date}? y/n: ')
    logger.info(f"User requested to delete {delete_distance}km run from {delete_date}. ID was {delete_id}.")
    if confirm_input.lower() == 'y':
        delete_query = f'DELETE FROM running_log WHERE id = {delete_id}'
        cursor.execute(delete_query)
        conn.commit
        print(f'{delete_distance}km run on {delete_date} deleted successfully.')
        logger.info(f"User confirmed {delete_distance}km run from {delete_date}. ID was {delete_id}.")
    else:
        print('Deletion cancelled, press Enter to return to the main menu...')
